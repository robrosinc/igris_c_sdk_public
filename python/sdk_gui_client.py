...
TWIST_PERIOD_S = 1.0 / 20.0  # 20 Hz walk velocity publish loop
TWIST_STEP_LIN = 0.05  # m/s per +/- click
TWIST_STEP_ANG = 0.1  # rad/s per +/- click
...
class AppState:
    ...
    # Walk velocity (Twist) — published on rt/cmd_vel at 20Hz while active.
    twist_publishing: bool = False
    twist_flushing: bool = False  # sending the post-stop zero burst
    twist_vx: float = 0.0  # linear.x  (m/s)
    twist_vy: float = 0.0  # linear.y  (m/s)
    twist_wz: float = 0.0  # angular.z (rad/s)
    twist_publish_count: int = 0
...
def call_init_hand(state: AppState, client: igc_sdk.IgrisC_Client) -> None:
    _spawn_service(state, "InitHand()", lambda: client.InitHand(SERVICE_TIMEOUT_MS))
...
# -- WalkTwist publishing thread -----------------------------------------------


def twist_publish_loop(
    state: AppState,
    publisher: igc_sdk.WalkTwistPublisher,
    stop_event: threading.Event,
) -> None:
    next_time = time.monotonic()
    seq = 0
    ...
    while not stop_event.is_set():
        ...
        with state.state_lock:
            active = state.twist_publishing
            # Falling edge (Stop): the callback already zeroed the velocity, so
            # keep publishing those zeros for TWIST_STOP_FLUSH_S.
            if not active and prev_active:
                flush_deadline = now_mono + TWIST_STOP_FLUSH_S
                state.twist_flushing = True
            if state.twist_flushing and (active or now_mono >= flush_deadline):
                state.twist_flushing = False
            send = active or state.twist_flushing
            prev_active = active
            if not send:
                continue
            vx = state.twist_vx
...
class Tags:
    ...
    twist_vx_text = "twist_vx_text"
    twist_vy_text = "twist_vy_text"
    twist_wz_text = "twist_wz_text"
    twist_publish_button = "twist_publish_button"
...
def build_center_panel(ctx: "GuiContext") -> None:
    with dpg.child_window(border=False, width=-1, height=-1):
        ...
        dpg.add_text("Roll-Pitch-Yaw (degree):", color=COLOR_LABEL)
        dpg.add_text("  R: 0.0000   P: 0.0000   Y: 0.0000", tag=Tags.imu_rpy)

        dpg.add_separator()
        dpg.add_text("Walk Velocity (rt/cmd_vel)", color=COLOR_HEADER)
        dpg.add_separator()
        dpg.add_text("Switch to walkmode first (Control Mode > Walkmode).", color=COLOR_YELLOW)

        # One steer axis row: label, [-], current value, [+].
        def _twist_row(label: str, axis: str, step: float, unit: str, value_tag: str) -> None:
            with dpg.group(horizontal=True):
                dpg.add_text(f"{label:<10}")
                dpg.add_button(
                    label="-", width=S(30), callback=lambda: ctx.on_twist_step(axis, -step)
                )
                dpg.add_text(f"  0.00 {unit}", tag=value_tag)
                dpg.add_button(
                    label="+", width=S(30), callback=lambda: ctx.on_twist_step(axis, step)
                )

        _twist_row("linear.x", "vx", TWIST_STEP_LIN, "m/s", Tags.twist_vx_text)
        _twist_row("linear.y", "vy", TWIST_STEP_LIN, "m/s", Tags.twist_vy_text)
        _twist_row("angular.z", "wz", TWIST_STEP_ANG, "rad/s", Tags.twist_wz_text)
        with dpg.group(horizontal=True):
            dpg.add_button(label="Zero", width=S(80), callback=ctx.on_twist_zero)
            dpg.add_button(
                label="Publish cmd_vel Start",
                tag=Tags.twist_publish_button,
                callback=ctx.on_toggle_twist_publish,
            )
        dpg.add_text("Status: idle", tag=Tags.twist_status, color=COLOR_RED)
...
def build_right_panel(ctx: "GuiContext") -> None:
    with dpg.child_window(border=False, width=-1, height=-1):
        ...
        dpg.add_separator()
        dpg.add_text("Hand", color=COLOR_HEADER)
        dpg.add_button(label="Init Hand", width=-1, height=S(32), callback=ctx.on_init_hand)
        ...
        dpg.add_button(label="Custom Mode 4", width=-1, height=S(32), callback=ctx.on_custom4)
        dpg.add_button(label="Custom Mode 5", width=-1, height=S(32), callback=ctx.on_custom5)
        dpg.add_button(label="Custom Mode 6", width=-1, height=S(32), callback=ctx.on_custom6)
        dpg.add_button(label="Custom Mode 7", width=-1, height=S(32), callback=ctx.on_custom7)
        dpg.add_button(label="Custom Mode 8", width=-1, height=S(32), callback=ctx.on_custom8)
        dpg.add_button(label="Custom Mode 9", width=-1, height=S(32), callback=ctx.on_custom9)
        dpg.add_button(label="Custom Mode 10", width=-1, height=S(32), callback=ctx.on_custom10)

        dpg.add_separator()
        dpg.add_text("Motion Presets", color=COLOR_HEADER)
        dpg.add_button(label="HOME", width=-1, height=S(32), callback=ctx.on_motion_home)
        dpg.add_button(
            label="WALKMODE STANDBY", width=-1, height=S(32), callback=ctx.on_motion_walk_standby
        )
...
class GuiContext:
    ...
    def on_twist_step(self, axis: str, delta: float) -> None:
        with self.state.state_lock:
            if axis == "vx":
                self.state.twist_vx += delta
            elif axis == "vy":
                self.state.twist_vy += delta
            elif axis == "wz":
                self.state.twist_wz += delta

    def on_twist_zero(self) -> None:
        with self.state.state_lock:
            self.state.twist_vx = 0.0
            self.state.twist_vy = 0.0
            self.state.twist_wz = 0.0

    def on_toggle_twist_publish(self) -> None:
        with self.state.state_lock:
            self.state.twist_publishing = not self.state.twist_publishing
            active = self.state.twist_publishing
            if not active:
                # Zero the velocity; the publish loop sends these zeros for 1s
                # so the robot decelerates to a halt.
                self.state.twist_vx = 0.0
                self.state.twist_vy = 0.0
                self.state.twist_wz = 0.0
        self.state.add_log(
            "WalkTwist publishing started (rt/cmd_vel, 20Hz)"
            if active
            else "WalkTwist stopped (sending zero velocity for 1s)"
        )
    ...
    def on_init_hand(self) -> None:
        self.request_confirm(
            "Init Hand",
            lambda: call_init_hand(self.state, self.client),
        )
    ...
    def on_custom4(self) -> None:
        self.request_confirm(
            "Custom Mode 4",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_4,
                "CUSTOM_MODE_4",
            ),
        )

    def on_custom5(self) -> None:
        self.request_confirm(
            "Custom Mode 5",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_5,
                "CUSTOM_MODE_5",
            ),
        )

    def on_custom6(self) -> None:
        self.request_confirm(
            "Custom Mode 6",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_6,
                "CUSTOM_MODE_6",
            ),
        )

    def on_custom7(self) -> None:
        self.request_confirm(
            "Custom Mode 7",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_7,
                "CUSTOM_MODE_7",
            ),
        )

    def on_custom8(self) -> None:
        self.request_confirm(
            "Custom Mode 8",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_8,
                "CUSTOM_MODE_8",
            ),
        )

    def on_custom9(self) -> None:
        self.request_confirm(
            "Custom Mode 9",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_9,
                "CUSTOM_MODE_9",
            ),
        )

    def on_custom10(self) -> None:
        self.request_confirm(
            "Custom Mode 10",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_CUSTOM_MODE_10,
                "CUSTOM_MODE_10",
            ),
        )
    ...
    def on_motion_walk_standby(self) -> None:
        self.request_confirm(
            "Motion Preset: WALKMODE STANDBY",
            lambda: call_control_mode(
                self.state,
                self.client,
                igc_sdk.ControlModeCommandType.CONTROL_MODE_CMD_MOTION_PRESET,
                "MOTION_PRESET(WALK_STANDBY)",
                "WALK_STANDBY",
                False,
            ),
        )
...
def _update_twist_ui(snapshot: dict) -> None:
    dpg.set_value(Tags.twist_vx_text, f"  {snapshot['twist_vx']:5.2f} m/s")
    dpg.set_value(Tags.twist_vy_text, f"  {snapshot['twist_vy']:5.2f} m/s")
    dpg.set_value(Tags.twist_wz_text, f"  {snapshot['twist_wz']:5.2f} rad/s")
    if snapshot["twist_publishing"]:
        dpg.set_value(
            Tags.twist_status, f"Status: PUBLISHING ({snapshot['twist_publish_count']} msgs)"
        )
        _set_text_color(Tags.twist_status, COLOR_GREEN)
        dpg.configure_item(Tags.twist_publish_button, label="Publish cmd_vel Stop")
    elif snapshot["twist_flushing"]:
        dpg.set_value(Tags.twist_status, "Status: stopping (zero velocity)...")
        _set_text_color(Tags.twist_status, COLOR_YELLOW)
        dpg.configure_item(Tags.twist_publish_button, label="Publish cmd_vel Start")
    else:
        dpg.set_value(Tags.twist_status, "Status: idle")
        _set_text_color(Tags.twist_status, COLOR_RED)
        dpg.configure_item(Tags.twist_publish_button, label="Publish cmd_vel Start")
...
def update_ui(state: AppState, channel: igc_sdk.ChannelFactory) -> None:
    with state.state_lock:
        snapshot = {
            ...
            "twist_vx": state.twist_vx,
            "twist_vy": state.twist_vy,
            "twist_wz": state.twist_wz,
            "twist_publishing": state.twist_publishing,
            "twist_flushing": state.twist_flushing,
            "twist_publish_count": state.twist_publish_count,
...
def _initialize_sdk(
    ...
    # Default QoS (~rclcpp::QoS(10)) matches the bridge's rt/cmd_vel subscriber.
    ...
    return channel, client, lowstate_sub, robotstate_sub, lowcmd_pub, twist_pub
...
def main() -> int:
    ...
    if sdk is None:
        return 1
    channel, client, lowstate_sub, robotstate_sub, lowcmd_pub, twist_pub = sdk
    ...
    twist_thread = threading.Thread(
        target=twist_publish_loop,
        args=(state, twist_pub, stop_event),
        name="twist-pub",
        daemon=True,
    )
    twist_thread.start()
    ...
    try:
        ...
        if twist_thread.is_alive():
            twist_thread.join(timeout=1.0)
        try:
            twist_pub.stop()
