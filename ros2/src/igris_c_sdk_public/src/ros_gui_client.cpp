/**
 * @file ros_gui_client.cpp
 * @brief ROS2 GUI client example mirroring the IGRIS SDK GUI layout
 *
...
#include <igris_c_sdk/msg/robot_state.hpp>
...
using LowState       = igris_c_sdk::msg::LowState;
using RobotState     = igris_c_sdk::msg::RobotState;
using WalkTwist      = igris_c_sdk::msg::WalkTwist;
using BmsInitSrv     = igris_c_sdk::srv::BmsInitCmd;
using TorqueSrv      = igris_c_sdk::srv::TorqueCmd;
using ControlModeSrv = igris_c_sdk::srv::ControlModeCommandRequest;
using MujocoSimSrv   = igris_c_sdk::srv::MujocoSimCmd;
using HandInitSrv    = igris_c_sdk::srv::HandInitRequest;
using igris_c_sdk_examples::resolveRobotNamespace;
using igris_c_sdk_examples::resolveRosDomainId;
using igris_c_sdk_examples::resolveRtName;
...
// Walk velocity (Twist) state — published on rt/cmd_vel at 20Hz while active.
// Steered with +/- step buttons (no multi-touch needed); switch the robot into
// walkmode before publishing.
static std::atomic<bool> g_twist_publishing(false);
static std::atomic<bool> g_twist_flushing(false);  // sending the post-stop zero burst
static std::atomic<double> g_twist_vx(0.0);        // linear.x  (m/s)
static std::atomic<double> g_twist_vy(0.0);        // linear.y  (m/s)
static std::atomic<double> g_twist_wz(0.0);        // angular.z (rad/s)
static std::atomic<uint32_t> g_twist_publish_count(0);
static constexpr double TWIST_STEP_LIN = 0.05;  // m/s per click
static constexpr double TWIST_STEP_ANG = 0.1;   // rad/s per click
// On Stop, keep publishing zero velocity for this long so the robot decelerates
// to a halt instead of holding the last commanded velocity.
static constexpr auto TWIST_STOP_FLUSH = std::chrono::seconds(1);
...
// Helper to convert RobotState::state to string. Mirrors the labels used by
// robros::lib::common::RobotControlState::toString() in igris_c_lib.
const char *RobotControlStateToString(uint8_t state) {
    switch (state) {
    case RobotState::ROBOT_STATE_NOT_READY:
        return "NOT_READY";
    case RobotState::ROBOT_STATE_IDLE:
        return "IDLE";
    case RobotState::ROBOT_STATE_STOP:
        return "STOP";
    case RobotState::ROBOT_STATE_LOW:
        return "LOW";
    case RobotState::ROBOT_STATE_HIGH_MOTION_ACTIVE:
        return "HIGH_MOTION_ACTIVE";
    case RobotState::ROBOT_STATE_HIGH_MOTION_STANDBY:
        return "HIGH_MOTION_STANDBY";
    case RobotState::ROBOT_STATE_WALK_LOW:
        return "WALK_LOW";
    case RobotState::ROBOT_STATE_WALK_HIGH_MOTION_ACTIVE:
        return "WALK_HIGH_MOTION_ACTIVE";
    case RobotState::ROBOT_STATE_WALK_HIGH_MOTION_STANDBY:
        return "WALK_HIGH_MOTION_STANDBY";
    case RobotState::ROBOT_STATE_WHOLE_BODY_CONTROL:
...
struct Ros2Clients {
    ...
    rclcpp::Client<BmsInitSrv>::SharedPtr bms;
    rclcpp::Client<TorqueSrv>::SharedPtr torque;
    rclcpp::Client<ControlModeSrv>::SharedPtr control_mode;
    rclcpp::Client<MujocoSimSrv>::SharedPtr mujoco_sim;
    rclcpp::Client<HandInitSrv>::SharedPtr hand_init;
};

template <typename ClientT, typename RequestT>
std::string CallServiceBlocking(const rclcpp::Node::SharedPtr &node, const ClientT &client, const std::shared_ptr<RequestT> &req,
                                const char *label) {
    (void)node;
    if (!client->wait_for_service(std::chrono::seconds(3))) {
        return std::string(label) + ": FAILED - service not available";
    }
    ...
    while (rclcpp::ok() && std::chrono::steady_clock::now() < deadline) {
        if (future.wait_for(std::chrono::milliseconds(20)) == std::future_status::ready) {
            auto res = future.get();
            return std::string(label) + ": " + (res->success ? "SUCCESS" : "FAILED") + " - " + res->message;
        }
    }
    {
        return std::string(label) + ": FAILED - request timeout";
    }
}

// Service call helpers (async versions to prevent GUI freeze)
void CallInitBmsAsync(Ros2Clients *clients, uint8_t type, const char *type_name) {
    ...
    std::thread([clients, type, type_name_copy]() {
        auto req        = std::make_shared<BmsInitSrv::Request>();
        req->request_id = "gui_bms_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        req->init       = type;
        AddLog(CallServiceBlocking(clients->node, clients->bms, req, (std::string("InitBms(") + type_name_copy + ")").c_str()));
        g_service_call_in_progress = false;
    }).detach();
}

void CallSetTorqueAsync(Ros2Clients *clients, uint8_t type, const char *type_name) {
    if (g_service_call_in_progress.load()) {
        AddLog("Service call already in progress, please wait...");
        return;
    }
    ...
    std::string type_name_copy(type_name);
    std::thread([clients, type, type_name_copy]() {
        auto req        = std::make_shared<TorqueSrv::Request>();
        req->request_id = "gui_torque_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        req->torque     = type;
        AddLog(CallServiceBlocking(clients->node, clients->torque, req, (std::string("SetTorque(") + type_name_copy + ")").c_str()));
        g_service_call_in_progress = false;
    }).detach();
}

void CallInitHandAsync(Ros2Clients *clients) {
    if (g_service_call_in_progress.load()) {
        AddLog("Service call already in progress, please wait...");
        return;
    }
    g_service_call_in_progress = true;
    AddLog("Calling InitHand()...");

    std::thread([clients]() {
        auto req        = std::make_shared<HandInitSrv::Request>();
        req->request_id = "gui_hand_init_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        AddLog(CallServiceBlocking(clients->node, clients->hand_init, req, "InitHand()"));
        g_service_call_in_progress = false;
    }).detach();
}

void CallSendControlModeCommandAsync(Ros2Clients *clients, uint8_t cmd_type, const char *label, const std::string &preset_id = "",
    ...
    std::thread([clients, cmd_type, label_copy, preset_id, is_cyclic]() {
        auto req          = std::make_shared<ControlModeSrv::Request>();
        req->request_id   = "gui_control_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        req->command_type = cmd_type;
        req->preset_id    = preset_id;
        req->is_cyclic    = is_cyclic;
        AddLog(CallServiceBlocking(clients->node, clients->control_mode, req,
                                   (std::string("ControlModeCommand(") + label_copy + ")").c_str()));
        g_service_call_in_progress = false;
    }).detach();
}

void CallSendMujocoSimCmdAsync(Ros2Clients *clients, uint8_t cmd_type, const char *label) {
    if (g_service_call_in_progress.load()) {
        AddLog("Service call already in progress, please wait...");
        return;
    }
    ...
    std::string label_copy(label);
    std::thread([clients, cmd_type, label_copy]() {
        auto req          = std::make_shared<MujocoSimSrv::Request>();
        req->request_id   = "gui_mujoco_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        req->command_type = cmd_type;
        AddLog(CallServiceBlocking(clients->node, clients->mujoco_sim, req, (std::string("MujocoSim(") + label_copy + ")").c_str()));
        g_service_call_in_progress = false;
    }).detach();
}
...
void LowCmdPublishThread(rclcpp::Publisher<LowCmd>::SharedPtr publisher) {
...

            {
                ...
                for (int mode_idx = 0; mode_idx < kParallelLinkCount; ++mode_idx) {
                    cmd.kinematic_modes[mode_idx] =
                        (g_parallel_space[static_cast<size_t>(mode_idx)] == static_cast<int>(SpaceSelection::JOINT))
                            ? LowCmd::KINEMATIC_MODE_PJS
                            : LowCmd::KINEMATIC_MODE_MS;
                }
...
// 20Hz WalkTwist publishing thread (rt/cmd_vel). Publishes the current
// linear.x / linear.y / angular.z values while g_twist_publishing is set.
void TwistPublishThread(rclcpp::Publisher<WalkTwist>::SharedPtr publisher) {
    ...
    while (g_running) {
        const bool active = g_twist_publishing.load();
        // Falling edge (Stop pressed): the toggle has already zeroed the
        // velocity, so keep publishing those zeros for TWIST_STOP_FLUSH.
        if (!active && prev_active) {
            flush_deadline   = std::chrono::steady_clock::now() + TWIST_STOP_FLUSH;
            g_twist_flushing = true;
        }
        if (g_twist_flushing && (active || std::chrono::steady_clock::now() >= flush_deadline)) {
            g_twist_flushing = false;
        }

        if (active || g_twist_flushing) {
            WalkTwist msg;
            msg.twist.linear.x  = g_twist_vx.load();
            msg.twist.linear.y  = g_twist_vy.load();
            msg.twist.angular.z = g_twist_wz.load();
            publisher->publish(msg);
            g_twist_publish_count++;
        }
...
int main(int argc, char **argv) {
    ...
    std::cout << "IGRIS-C ROS2 Dev Tool" << std::endl;
    std::cout << "Make sure the robot controller is running!\n" << std::endl;

    igris_c_sdk_examples::initRclcpp(argc, argv);
    auto node = rclcpp::Node::make_shared("igris_c_ros_gui_client");
    ...
    GLFWwindow *window = glfwCreateWindow(initial_width, initial_height, "IGRIS-C ROS2 Dev Tool", NULL, NULL);
    if (window == NULL) {
    ...
    const std::size_t ros_domain_id   = resolveRosDomainId();

    auto lowstate_sub   = node->create_subscription<LowState>(resolveRtName(robot_namespace, "rt/lowstate"), rclcpp::SensorDataQoS(),
                                                              [](const LowState &state) { LowStateCallback(state); });
    auto robotstate_sub = node->create_subscription<RobotState>(resolveRtName(robot_namespace, "rt/robotstate"), rclcpp::QoS(10),
                                                                [](const RobotState &state) { RobotStateCallback(state); });
    auto lowcmd_pub     = node->create_publisher<LowCmd>(resolveRtName(robot_namespace, "rt/lowcmd"), rclcpp::SensorDataQoS());
    auto twist_pub      = node->create_publisher<WalkTwist>(resolveRtName(robot_namespace, "rt/cmd_vel"), rclcpp::QoS(10));
    Ros2Clients clients;
    clients.node         = node;
    clients.bms          = node->create_client<BmsInitSrv>(resolveRtName(robot_namespace, "rt/service/bms_init"));
    clients.torque       = node->create_client<TorqueSrv>(resolveRtName(robot_namespace, "rt/service/torque"));
    clients.control_mode = node->create_client<ControlModeSrv>(resolveRtName(robot_namespace, "rt/service/control_mode"));
    clients.mujoco_sim   = node->create_client<MujocoSimSrv>(resolveRtName(robot_namespace, "rt/service/mujoco_sim"));
    clients.hand_init    = node->create_client<HandInitSrv>(resolveRtName(robot_namespace, "rt/service/hand_init"));
    AddLog("ROS2 GUI Client initialized successfully");

    // Start LowCmd publishing thread
    std::thread publish_thread(LowCmdPublishThread, lowcmd_pub);

    // Start WalkTwist publishing thread (rt/cmd_vel)
    std::thread twist_thread(TwistPublishThread, twist_pub);
    ...
    while (!glfwWindowShouldClose(window) && g_running && rclcpp::ok()) {
        ...
        ImGui::BeginChild("CenterPanel", ImVec2(center_panel_width, top_panel_height), true);
        {
            ...
            // ROS2 Connection Status — fixed-width sub-frame.
            ImGui::BeginChild("DdsConnInfo", ImVec2(0.0f, 0), ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
            {
                SectionHeader("ROS2 Connection Status");

                ImGui::PushTextWrapPos(ImGui::GetContentRegionAvail().x);
                ImGui::Text("Domain:    %zu", ros_domain_id);
                ImGui::Text("Namespace: %s", robot_namespace.empty() ? "(none)" : robot_namespace.c_str());
                ...
                if (is_connected) {
                    ImGui::TextColored(ImVec4(0, 1, 0, 1), "Status: Connected");
                    {
                        ...
                        is_sim_env = (g_latest_robotstate.environment == RobotState::ROBOT_ENV_SIM);
                ...
                if (is_connected && is_sim_env) {
                    ...
                    if (ScaledButton("Pause", ImVec2(button_width, action_btn_height))) {
                        CallSendMujocoSimCmdAsync(&clients, MujocoSimSrv::Request::MUJOCO_SIM_PAUSE, "PAUSE");
                    }
                    ImGui::SameLine();
                    if (ScaledButton("Resume", ImVec2(button_width, action_btn_height))) {
                        CallSendMujocoSimCmdAsync(&clients, MujocoSimSrv::Request::MUJOCO_SIM_RESUME, "RESUME");
                    }
                    if (ScaledButton("Reload Model", ImVec2(button_width, action_btn_height))) {
                        CallSendMujocoSimCmdAsync(&clients, MujocoSimSrv::Request::MUJOCO_SIM_RELOAD, "RELOAD");
                    }
                    ImGui::SameLine();
                    if (ScaledButton("Reset State", ImVec2(button_width, action_btn_height))) {
                        CallSendMujocoSimCmdAsync(&clients, MujocoSimSrv::Request::MUJOCO_SIM_RESET, "RESET");
                    }
                }
            }
            ImGui::EndChild();
            ...
            ImGui::BeginChild("ImuStateBox", ImVec2(0.0f, 0), ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
            {
                ...
                if (g_first_state_received) {
                    ...
                    ImGui::TextColored(label_color, "Roll-Pitch-Yaw (degree):");
            ...
            // Walk Velocity (Twist) — publishes WalkTwist on rt/cmd_vel at 20Hz.
            ImGui::BeginChild("WalkTwistBox", ImVec2(0.0f, 0), ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border);
            {
                SectionHeader("Walk Velocity (rt/cmd_vel)");
                ImGui::TextColored(ImVec4(1, 1, 0, 1), "Switch to walkmode first (Control Mode > Walkmode).");
                ImGui::Spacing();

                const float step_btn_w = 36.0f * ui_scale;
                // One steer axis row: label, [-], current value, [+].
                auto twist_row = [&](const char *label, std::atomic<double> &val, double step, const char *unit) {
                    char minus_id[40];
                    char plus_id[40];
                    std::snprintf(minus_id, sizeof(minus_id), "-##%s", label);
                    std::snprintf(plus_id, sizeof(plus_id), "+##%s", label);
                    ImGui::Text("%-10s", label);
                    ImGui::SameLine();
                    if (ImGui::Button(minus_id, ImVec2(step_btn_w, 0))) {
                        val.store(val.load() - step);
                    }
                    ImGui::SameLine();
                    ImGui::Text("% .2f %-5s", val.load(), unit);
                    ImGui::SameLine();
                    if (ImGui::Button(plus_id, ImVec2(step_btn_w, 0))) {
                        val.store(val.load() + step);
                    }
                };
                twist_row("linear.x", g_twist_vx, TWIST_STEP_LIN, "m/s");
                twist_row("linear.y", g_twist_vy, TWIST_STEP_LIN, "m/s");
                twist_row("angular.z", g_twist_wz, TWIST_STEP_ANG, "rad/s");

                ImGui::Spacing();
                if (ScaledButton("Zero", ImVec2(step_btn_w * 3.0f, action_btn_height))) {
                    g_twist_vx.store(0.0);
                    g_twist_vy.store(0.0);
                    g_twist_wz.store(0.0);
                }
                ImGui::SameLine();
                if (!g_twist_publishing) {
                    if (ScaledButton("Publish cmd_vel Start", ImVec2(-1, action_btn_height))) {
                        g_twist_publishing = true;
                        AddLog("WalkTwist publishing started (rt/cmd_vel, 20Hz)");
                    }
                } else {
                    if (ScaledButton("Publish cmd_vel Stop", ImVec2(-1, action_btn_height))) {
                        // Zero the velocity; the publish thread sends these zeros
                        // for 1s so the robot decelerates to a halt.
                        g_twist_vx.store(0.0);
                        g_twist_vy.store(0.0);
                        g_twist_wz.store(0.0);
                        g_twist_publishing = false;
                        AddLog("WalkTwist stopped (sending zero velocity for 1s)");
                    }
                }
                if (g_twist_publishing) {
                    ImGui::TextColored(ImVec4(0, 1, 0, 1), "Status: PUBLISHING (%u msgs)", g_twist_publish_count.load());
                } else if (g_twist_flushing) {
                    ImGui::TextColored(ImVec4(1, 1, 0, 1), "Status: stopping (zero velocity)...");
                } else {
        ...
        ImGui::BeginChild("RightPanel", ImVec2(right_panel_width, top_panel_height), true);
        {
            ...
            if (ScaledButton("Init BMS", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Init BMS", [&clients]() { CallInitBmsAsync(&clients, BmsInitSrv::Request::BMS_INIT, "BMS_INIT"); });
            }
            if (ScaledButton("Init Motor", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Init Motor",
                                      [&clients]() { CallInitBmsAsync(&clients, BmsInitSrv::Request::MOTOR_INIT, "MOTOR_INIT"); });
            }
            if (ScaledButton("Init BMS + Motor", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Init BMS + Motor", [&clients]() {
                    CallInitBmsAsync(&clients, BmsInitSrv::Request::BMS_AND_MOTOR_INIT, "BMS_AND_MOTOR_INIT");
                });
            }
            if (ScaledButton("BMS OFF", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("BMS OFF",
                                      [&clients]() { CallInitBmsAsync(&clients, BmsInitSrv::Request::BMS_INIT_NONE, "BMS_INIT_NONE"); });
            }
            if (ScaledButton("Torque ON", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Torque ON",
                                      [&clients]() { CallSetTorqueAsync(&clients, TorqueSrv::Request::TORQUE_ON, "TORQUE_ON"); });
            }
            if (ScaledButton("Torque OFF", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Torque OFF",
                                      [&clients]() { CallSetTorqueAsync(&clients, TorqueSrv::Request::TORQUE_OFF, "TORQUE_OFF"); });
            }

            // --- Hand -----------------------------------------------------------
            SubsectionHeader("Hand");
            if (ScaledButton("Init Hand", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Init Hand", [&clients]() { CallInitHandAsync(&clients); });
            }

            // --- Control Mode ---------------------------------------------------
            SubsectionHeader("Control Mode");
            if (ScaledButton("High-Level", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Control Mode: High-Level", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_HIGH_LEVEL_JOINT_CONTROL,
                                                    "HIGH_LEVEL_JOINT_CONTROL");
                });
            }
            if (ScaledButton("Low-Level", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Control Mode: Low-Level", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_LOW_LEVEL_JOINT_CONTROL,
                                                    "LOW_LEVEL_JOINT_CONTROL");
                });
            }
            if (ScaledButton("Joint Position Hold", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Joint Position Hold", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_JOINT_POSITION_HOLD,
                                                    "JOINT_POSITION_HOLD");
                });
            }
            if (ScaledButton("Motion Stop", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Motion Stop", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_MOTION_STOP, "MOTION_STOP");
                });
            }
            if (ScaledButton("Walkmode", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Walkmode", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_WALKMODE_ON, "WALKMODE_ON");
                });
            }
            if (ScaledButton("Low-Level Walkmode", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Low-Level Walkmode", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_LOW_LEVEL_WALKMODE_ON,
                                                    "LOW_LEVEL_WALKMODE_ON");
                });
            }
            if (ScaledButton("High-Level Walkmode", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("High-Level Walkmode", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_HIGH_LEVEL_WALKMODE_ON,
            ...
            if (ScaledButton("Custom Mode 1", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 1", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_1, "CUSTOM_MODE_1");
                });
            }
            if (ScaledButton("Custom Mode 2", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 2", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_2, "CUSTOM_MODE_2");
                });
            }
            if (ScaledButton("Custom Mode 3", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 3", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_3, "CUSTOM_MODE_3");
                });
            }
            if (ScaledButton("Custom Mode 4", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 4", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_4, "CUSTOM_MODE_4");
                });
            }
            if (ScaledButton("Custom Mode 5", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 5", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_5, "CUSTOM_MODE_5");
                });
            }
            if (ScaledButton("Custom Mode 6", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 6", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_6, "CUSTOM_MODE_6");
                });
            }
            if (ScaledButton("Custom Mode 7", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 7", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_7, "CUSTOM_MODE_7");
                });
            }
            if (ScaledButton("Custom Mode 8", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 8", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_8, "CUSTOM_MODE_8");
                });
            }
            if (ScaledButton("Custom Mode 9", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 9", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_9, "CUSTOM_MODE_9");
                });
            }
            if (ScaledButton("Custom Mode 10", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Custom Mode 10", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_CUSTOM_MODE_10, "CUSTOM_MODE_10");
                });
            }
            ...
            if (ScaledButton("HOME", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Motion Preset: HOME", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_MOTION_PRESET,
                                                    "MOTION_PRESET(HOME)", "HOME", false);
                });
            }
            if (ScaledButton("WALKMODE STANDBY", ImVec2(-1, action_btn_height))) {
                RequestServiceConfirm("Motion Preset: WALKMODE STANDBY", [&clients]() {
                    CallSendControlModeCommandAsync(&clients, ControlModeSrv::Request::CONTROL_MODE_CMD_MOTION_PRESET,
                                                    "MOTION_PRESET(WALK_STANDBY)", "WALK_STANDBY", false);
                });
            }
        }
        ImGui::EndChild();
    ...
    // Wait for publish threads to finish
    if (publish_thread.joinable()) {
        publish_thread.join();
    }
    if (twist_thread.joinable()) {
        twist_thread.join();
    }
