...
constexpr const char *kImuTopic = "sensor/d435_imu";
...
struct ImuStream {
    std::string name;
    Subscriber<IMUState> subscriber;
    StreamStats stats;
    ...
    explicit ImuStream(const std::string &topic_name) : name(topic_name), subscriber(topic_name, QosProfile::SensorData()) {}
};
}  // namespace
...
int main(int argc, char **argv) {
    ...
    ImuStream imu(kImuTopic);
    if (!imu.subscriber.init([imu = &imu](const IMUState &msg) {
            std::lock_guard<std::mutex> lock(imu->mutex);
            update_stats(imu->stats, 52);
            imu->accel    = msg.accelerometer();
            imu->gyro     = msg.gyroscope();
            imu->rpy      = msg.rpy();
            imu->has_data = true;
        })) {
        std::cerr << "Failed to initialize subscriber for topic: " << imu.name << std::endl;
        return 1;
    }
    ...
    while (!g_stop.load()) {
...

        {
            ...
            std::cout << "\n" << imu.name << ": ";
            if (imu.has_data) {
                std::cout << std::fixed << std::setprecision(1) << imu.stats.fps << " Hz\n";
                std::cout << std::setprecision(3) << "  accel [m/s^2]: " << imu.accel[0] << ", " << imu.accel[1] << ", " << imu.accel[2]
                          << "\n";
                std::cout << "  gyro  [rad/s]: " << imu.gyro[0] << ", " << imu.gyro[1] << ", " << imu.gyro[2] << "\n";
