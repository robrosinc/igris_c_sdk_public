...
int main(int argc, char **argv) {
    ...
    // Create subscriber
    Subscriber<LowState> state_sub("rt/lowstate", QosProfile::SensorData());
    ...
    // Create publisher
    Publisher<LowCmd> cmd_pub("rt/lowcmd", QosProfile::SensorData());
    if (!cmd_pub.init()) {
    ...
    while (g_running) {
        // Create command
        LowCmd cmd;
        cmd.kinematic_modes().fill(KinematicMode::PJS);  // Joint Space (전체 적용)
