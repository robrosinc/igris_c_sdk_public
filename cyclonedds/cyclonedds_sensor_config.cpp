...
constexpr const char *kStereoConfigRequestTopic    = "sensor/stereo_config_request";
constexpr const char *kRealsenseConfigRequestTopic = "sensor/realsense_config_request";
constexpr const char *kMonoConfigRequestTopic      = "sensor/mono_config_request";
constexpr const char *kCameraConfigResponseTopic   = "sensor/camera_config_response";

std::atomic<uint64_t> g_request_seq{0};

std::string apply_namespace_to_topic(const std::string &ns, const std::string &topic) {
    if (ns.empty()) {
        return topic;
    }
    return ns + "/" + topic;
...
std::string make_request_id() {
    ...
    return "camera_cfg_req_" + std::to_string(now_ms) + "_" + std::to_string(seq);
}
...
int main(int argc, char **argv) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <domain_id> <namespace> [timeout_ms]\n";
        std::cerr << "       " << argv[0] << " 10 \"\" 60000 (no namespace)\n";
        return 1;
    }
    ...
    if (argc > 3) {
        auto parsed = parse_int_arg(argv[3]);
        if (parsed.has_value() && *parsed > 0) {
            timeout_ms = *parsed;
        }
    }

    std::cout << "DDS domain_id=" << domain_id << ", namespace=" << ns << ", timeout_ms=" << timeout_ms << "\n";

    dds::domain::DomainParticipant participant(domain_id);
    dds::pub::Publisher publisher(participant);
    dds::sub::Subscriber subscriber(participant);

    const std::string stereo_topic    = apply_namespace_to_topic(ns, kStereoConfigRequestTopic);
    const std::string realsense_topic = apply_namespace_to_topic(ns, kRealsenseConfigRequestTopic);
    const std::string mono_topic      = apply_namespace_to_topic(ns, kMonoConfigRequestTopic);
    const std::string response_topic  = apply_namespace_to_topic(ns, kCameraConfigResponseTopic);
    dds::topic::Topic<StereoConfigRequest> stereo_req_topic(participant, stereo_topic);
    dds::topic::Topic<RealsenseConfigRequest> realsense_req_topic(participant, realsense_topic);
    dds::topic::Topic<MonoConfigRequest> mono_req_topic(participant, mono_topic);
    dds::topic::Topic<ServiceResponse> res_topic(participant, response_topic);
