/**
 * @file cyclonedds_hand_gui_client.cpp
...
int main(int argc, char **argv) {
    ...
    std::cout << "Initializing ChannelFactory (namespace=\"" << ns << "\")..." << std::endl;
    ...
    Subscriber<HandState> handstate_sub("rt/handstate", QosProfile::SensorData());
    ...
    Publisher<HandInitRequest> hand_init_req_pub("rt/service/hand_init/request", QosProfile::Services());
    if (!hand_init_req_pub.init()) {
        std::cerr << "Failed to initialize HandInit request publisher" << std::endl;
        return 1;
    }
    Subscriber<ServiceResponse> hand_init_res_sub("rt/service/hand_init/response", QosProfile::Services());
