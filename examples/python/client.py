...
if len(sys.argv) != 3:
    print(f"Usage: {sys.argv[0]} <domain_id> <namespace>", file=sys.stderr)
    print(f"Example: {sys.argv[0]} 0 robot1", file=sys.stderr)
    sys.exit(1)
...
def call_send_control_mode_command(
    ...
    result = "SUCCESS" if res.success() else "FAILED" + " - " + res.message()
    print(f"SendControlModeCommand({label}) result: {result}")
